...
pub fn package_manifest(
    workspace_directory: &std::path::PathBuf,
) -> Result<vouch_lib::schema::PackageManifest> {
    let mut files = Vec::new();
    collect_package_manifest_files(workspace_directory, workspace_directory, &mut files)?;
    files.sort();
    Ok(vouch_lib::schema::PackageManifest { files })
}

fn collect_package_manifest_files(
    root: &std::path::Path,
    current: &std::path::Path,
    files: &mut Vec<vouch_lib::schema::PackageManifestFile>,
) -> Result<()> {
    for entry in std::fs::read_dir(current)? {
        ...
        files.push(vouch_lib::schema::PackageManifestFile {
...
mod tests {
    ...
    fn package_manifest_lists_regular_files_with_sizes() -> Result<()> {
        ...
        assert_eq!(
            manifest.files,
            vec![
                vouch_lib::schema::PackageManifestFile {
